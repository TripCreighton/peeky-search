...
`SURVEY_BUDGET` is **not measured** — every published number describes the search budget. The metrics that would judge it (`sourcePrecision`, `canonicalMrr`, `badRate`) are page-level and already exist, so it is scoreable against the current labels as soon as a corpus is recorded.
